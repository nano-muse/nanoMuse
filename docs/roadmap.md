...
1. **Muse-style.** One agent, not a toolbox: a name and a face, a first conversation, a feed written for you, goals worked on in the background, memory you can read and edit, an approval before anything you could not undo. Meta's Muse (September 2026) serves this kind of agent from a cloud VM per user through thin clients (iOS, Android, the web, WhatsApp, a Mac app); nanoMuse is this kind of agent running on hardware of yours, where you say. *In the app since 0.1.2; complete through 0.1.11.*
...
| 0.1.7 | Welcome | The first run: a welcome screen with the three steps — add a provider, pick from the models it serves, meet the agent — in front of a chat that could not answer without them. |
| 0.1.8 | Polish | The shell finished: the type scale, the one-row composer pill, only the name under the face, grey reply bubbles, and the settings pages restyled to match on every screen. |
| 0.1.9 | Portrait | Changing the face from the chat: "change your avatar to…", four takes to pick from, poses, a share card; the agent's page behind the face; five avatar sizes; the name pill re-measured. |
...
| 0.1.12 | Hands | **The screen as a hand.** With *Settings → Hands* on, the agent looks at a screenshot — never the accessibility tree — decides one action, taps, types or swipes, and looks again; the accessibility service is only the hand. A floating capsule shows each step with *Stop*. The ladder is in the prompt: a skill, CLI, MCP server or the browser first, the screen last, and the agent says so before it starts. Logins, passwords, codes and protected pages are handed to you. The same `RiskGate` approvals before paying, sending or deleting, with the button's label and the app's name on the card. Off by default. Tried on a phone; the two reference tasks (12306's seats for a route; a reply that stops at the send button) are the ones to start with. |
| 0.1.13 | Reach | **The phone drives your computer.** A small companion on the PC (`host/nanomuse_host.py`: one file, standard library, Linux / macOS / Windows) that the app pairs with by a six-digit code on the same network; a sentence on the phone runs there — the shell, the files, the browser, a picture of the screen — and the results and the approvals come back to the phone, judged by the same `ShellGuard`. One way, phone to computer; no TLS yet. The pairing, the computer list and `--on <computer>` are the pieces every later device reuses. |
| 0.1.14 | Home | A fix: on a phone, every "open this session" entry point — notifications, the tool capsule, Hands bringing the app back — pushed the upstream OpenMinis chat screen over the home shell. `HomeShell.openSession` takes them to the shell instead. |
| 0.1.15 | Stage | **Watching the hands.** `HandsStage`: a glow along the screen's edges while the run is on, a turning ring with the action's name where the model is about to tap, a ripple as the finger lands, the ring riding along a swipe — UI-TARS-desktop's ScreenMarker redrawn on an Android Canvas, never in a screenshot. The capsule lets injected gestures through (`FLAG_NOT_TOUCHABLE` for the length of each) and dodges when it covers the target. App-icon quick actions open inside the home shell. |
| 0.1.16 | Palette | **The models the key can use.** `ImageGen.availableModels` lists the provider's image models from the same `/models` fetch the chat models come from (by catalogue modality or by name), recommended first; `MediaModels.checkVideoModels` probes the known Model Studio video models against the key without making a video. Drawing on Model Studio goes through the native `multimodal-generation` endpoint (`callDashScope`, shared with the poses) — the compatible host has no `images/generations`. `VideoGen` speaks both MiniMax and Wan bodies. |
