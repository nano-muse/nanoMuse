...
| 0.1.12 | Hands | **The screen as a hand — a working demo.** With *Settings → Hands* on, the agent looks at a screenshot — never the accessibility tree — decides one action, taps, types or swipes, and looks again; the accessibility service is only the hand. A floating capsule shows each step with *Stop*. The ladder is in the prompt: a skill, CLI, MCP server or the browser first, the screen last, and the agent says so before it starts. Logins, passwords, codes and protected pages are handed to you. The same `RiskGate` approvals before paying, sending or deleting, with the button's label and the app's name on the card. Off by default. Built and unit-tested; the two demo tasks (12306's seats for a route; a reply that stops at the send button) are to be driven on a device. |
| 0.1.13 | Reach | **The phone drives your computer — a working demo.** A small companion on the PC (`host/nanomuse_host.py`: one file, standard library, Linux / macOS / Windows) that the app pairs with by a six-digit code on the same network; a sentence on the phone runs there — the shell, the files, the browser, a picture of the screen — and the results and the approvals come back to the phone, judged by the same `ShellGuard`. One way, phone to computer; no TLS yet. The pairing, the computer list and `--on <computer>` are the pieces every later device reuses. |
