...
const zhCN: Record<string, string> = {
  ...
  "Set it up": "设置一下",
  "Three steps, two minutes.": "三步，两分钟。",
  "Give it a name, a face and a way of talking.": "给它一个名字、一张脸和一种说话方式。",
  "Named {name}. Tap to change.": "叫 {name}。点击修改。",
  "Add a model": "添加模型",
  ...
  "Start": "开始",
  "Open the chat and ask for the first thing.": "打开聊天，说出第一件事。",
  "Needs a model first.": "先要有一个模型。",
  "Add a model to start": "添加模型后开始",
  "optional": "可选",
  "The name comes first. Then a face, a tagline and how it talks — and what it should call you.": "先起名字。然后是一张脸、一句标语、它怎么说话——以及它怎么称呼你。",
  "That's {name}": "就叫 {name}",
  "1–20 characters. Empty means nanoMuse.": "1–20 个字符。留空即 nanoMuse。",
  "A line under the name, e.g. Your day, sorted.": "名字下面的一句话，例如：你的一天，安排好了。",
  "Tagline": "标语",
  "More names": "换一批名字",
  "Tone": "语气",
  "Formal": "正式",
  ...
  "Playful": "活泼",
  "Concise": "简洁",
  "How much it says": "话说多少",
  "Short replies": "回答简短",
  "Detailed": "详细",
  "Bullet points": "要点列表",
  "Anything else about how it should be": "还有什么想让它成为的样子",
  "e.g. A little witty. Uses metric units. Never uses emoji.": "例如：有点风趣。用公制单位。从不用 emoji。",
  "OpenAI-compatible · Chat Completions": "OpenAI 兼容 · Chat Completions",
  "Responses API": "Responses API",
  "Local or your own endpoint": "本地或你自己的端点",
  "/v1 is added when the URL has no path.": "URL 没有路径时会自动补上 /v1。",
  "The endpoint that serves /chat/completions, for example http://127.0.0.1:8000/v1.": "提供 /chat/completions 的端点，例如 http://127.0.0.1:8000/v1。",
  "{n} models from the endpoint": "端点返回了 {n} 个模型",
  "Asking the endpoint…": "正在询问端点…",
  "Could not list the endpoint's models; these are the usual ones.": "无法列出端点的模型；这些是常见的几个。",
  "Get a key from {vendor}": "去 {vendor} 获取密钥",
  "No key": "无需密钥",
  ...
  "Unrestricted": "无限制",
  "Optimised": "已优化",
  "Optimised means Android may freeze the agent after a while with the screen off; routines and check-ins then wait until the phone wakes.": "「已优化」意味着熄屏一段时间后安卓可能冻结 Agent；例行任务和提醒要等手机醒来才跑。",
  "Display over other apps": "显示在其他应用上层",
  "Allowed": "已允许",
  "Not allowed": "未允许",
  "Lets the agent bring an app to the front from the background and show its status capsule while it works in other apps.": "让 Agent 能从后台把应用调到前台，并在其他应用里工作时显示状态胶囊。",
  "Alarms & reminders": "闹钟和提醒",
  "Exact": "精确",
  "Approximate": "大致",
  "With exact alarms a reminder fires at the minute you named even when the phone is asleep; without them Android may deliver it up to a quarter of an hour late.": "有精确闹钟，提醒会在你说的那一分钟响起，哪怕手机在睡眠；没有的话安卓可能晚最多一刻钟送达。",
  "Start after a reboot": "重启后自动启动",
  "Bring the runtime back when the phone restarts, so routines and check-ins do not stop.": "手机重启后把运行时带回来，例行任务和提醒不中断。",
  "Reconnect to your computer when the phone restarts, so approvals keep arriving.": "手机重启后重新连上你的电脑，审批照常送达。",
  "On this phone's Android": "这台手机的安卓系统",
  "Open the auto-start settings": "打开自启动设置",
  "Xiaomi / Redmi / POCO: Settings → Apps → Manage apps → {name} → Autostart on; Battery saver → No restrictions; Other permissions → Display pop-up windows while running in the background on.": "小米 / Redmi / POCO：设置 → 应用设置 → 应用管理 → {name} → 自启动打开；省电策略 → 无限制；其他权限 → 后台弹出界面 允许。",
  "Huawei: Settings → Battery → App launch → {name} → Manage manually, all three on (auto-launch, secondary launch, run in background); Settings → Apps → {name} → Battery → Launch: manage manually.": "华为：设置 → 电池 → 应用启动管理 → {name} → 手动管理，三项全开（自动启动、关联启动、后台活动）；设置 → 应用 → {name} → 电池 → 启动管理：手动管理。",
  "Honor: Settings → Battery → App launch → {name} → Manage manually, all three on; Battery → More battery settings → Stay connected while asleep on.": "荣耀：设置 → 电池 → 应用启动管理 → {name} → 手动管理，三项全开；电池 → 更多电池设置 → 休眠时始终保持网络连接 打开。",
  "OPPO / realme / OnePlus: Settings → Battery → More settings → Optimise battery use → {name} → Don't optimise; Settings → Apps → {name} → Battery usage → Allow background activity, Allow auto launch.": "OPPO / realme / 一加：设置 → 电池 → 更多设置 → 应用耗电优化 → {name} → 不优化；设置 → 应用 → {name} → 耗电管理 → 允许后台活动、允许自动启动。",
  "vivo / iQOO: i Manager → App manager → Autostart → {name} on; Settings → Battery → Background power consumption management → {name} → Allow high background power consumption.": "vivo / iQOO：i 管家 → 应用管理 → 自启动 → {name} 打开；设置 → 电池 → 后台耗电管理 → {name} → 允许后台高耗电。",
  "Samsung: Settings → Battery → Background usage limits → {name} not in Sleeping or Deep sleeping apps (add it to Never sleeping apps); turn off Adaptive battery if it keeps stopping.": "三星：设置 → 电池 → 后台使用限制 → {name} 不在「休眠应用」或「深度休眠应用」里（加入「永不休眠的应用」）；如果还是被停，关闭自适应电池。",
  "Meizu: Settings → Apps → {name} → Background management → Keep running in the background; Battery → App power management → Allow.": "魅族：设置 → 应用管理 → {name} → 后台管理 → 允许后台运行；电池 → 应用耗电管理 → 允许。",
  "Crashes are written to a file on this phone and nowhere else. Export bundles them with the runtime's log for someone you choose.": "崩溃只写到这台手机上的文件里，不上报任何地方。导出会把它们和运行时日志打成一个包，发给你选的人。",
  "Export logs": "导出日志",
