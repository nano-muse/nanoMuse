...
Every stage of Phase 1 is a version — `0.1.1`, `0.1.2`, … `0.1.9`, then `0.2.0`. Version names stay plain numbers (the in-app update check compares them); each release also carries a one-word English codename (Foundation, Identity, Home, Guardrails, Memory, Avatar, Welcome, Polish, Portrait, Motion, Hatch), and the release title is `nanoMuse <version> · <Codename>`. `scripts/release-apk.sh <version>` builds the release APK, verifies the signature, writes the sha256 and creates the GitHub release (marked Latest; `--prerelease` for one that is not) with `docs/releases/v<version>.md` as the notes — English first, the same notes in Chinese in a collapsed block at the end; `docs/release-notes-template.md` is the shape. The signing key is one key for every version so an update installs over the previous one; it is not in the repository and not in CI.
