...
def edit(path, subs, must=True):
    ...
    for pat, rep in subs:
        ...
        if (
            ...
            and must
            and not re.search(
                re.escape(rep) if isinstance(rep, str) and "\\" not in rep else "(?!)", s
            )
        ):
...
edit(
    ROOT / "src/android/gradle.properties",
    [
...
edit(
    ...
    [(r'android:targetPackage="com\.openminis\.app"', f'android:targetPackage="{APP_ID}"')],
)
edit(
    MAIN / "java/com/openminis/app/accessibility/MinisAccessibilityService.kt",
    [
        (
...
for f in sorted(glob.glob(str(MAIN / "res/values*/strings.xml"))):
    ...
    if s2 != s:
        ...
        ET.fromstring(s2.encode("utf-8"))  # must still parse
        print(f"  {p.parent.name}: {len(re.findall(r'\\bMinis\\b(?!Skills)', s))} replaced")

print("== soul defaults")
edit(
    MAIN / "java/com/openminis/app/agent/SoulStore.kt",
    [
...
def transform(d, k=0.72, off=18.0):
    ...
    def flush():
        ...
        if nums:
            out.append(
                " ".join(
                    f"{off + k * float(v):.2f}" if i % 2 == 0 else f"{off + k * float(v):.2f}"
    ...
    flush()
    return (
        "".join(t if t in "MLCZ" else t + " " for t in out)
        .replace(" M", "M")
        .replace(" L", "L")
        .replace(" C", "C")
        .replace(" Z", "Z")
...
(MAIN / "res/drawable/ic_launcher_foreground_nm_dark.xml").write_text(
    vector(' android:fillColor="#FFFFFFFF" />'), encoding="utf-8"
)
(MAIN / "res/drawable/ic_launcher_monochrome.xml").write_text(
...
(MAIN / "res/mipmap-anydpi-v26/ic_launcher.xml").write_text(
    adaptive("@color/ic_launcher_background", "@drawable/ic_launcher_foreground_nm"),
    encoding="utf-8",
)
(MAIN / "res/mipmap-anydpi-v26/ic_launcher_classic_light.xml").write_text(
    adaptive("@color/ic_launcher_bg_force_light", "@drawable/ic_launcher_foreground_nm"),
    encoding="utf-8",
)
(MAIN / "res/mipmap-anydpi-v26/ic_launcher_classic_dark.xml").write_text(
    adaptive("@color/ic_launcher_bg_force_dark", "@drawable/ic_launcher_foreground_nm_dark"),
    encoding="utf-8",
)
for f, subs in [
    (
        MAIN / "res/values/ic_launcher_background.xml",
        [(r'(name="ic_launcher_background">)#[0-9A-Fa-f]+', r"\1#FFFFFF")],
    ),
    (
        MAIN / "res/values-night/ic_launcher_background.xml",
        [(r'(name="ic_launcher_background">)#[0-9A-Fa-f]+', r"\1#FFFFFF")],
    ),
    (
        MAIN / "res/values/ic_launcher_alt_colors.xml",
        [
            (r'(name="ic_launcher_bg_force_light">)#[0-9A-Fa-f]+', r"\1#FFFFFF"),
            (r'(name="ic_launcher_bg_force_dark">)#[0-9A-Fa-f]+', r"\1#0F1B33"),
        ],
    ),
]:
...
(ROOT / "src/android/local.properties").write_text(
    "sdk.dir=/ssd/software/android-studio/Android/SDK\n"
)
print("done")
