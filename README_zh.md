...
| | |
|---|---|
| **动手做事** | Linux shell、浏览器、MCP 服务、[Agent Skills](https://agentskills.io) 格式的技能；开了无障碍服务后，还能用 OpenMinis 自带的无障碍 CLI，通过屏幕操作手机上的 App（0.1.12 会把它做成一双正式的手：视觉、梯子、接管、审批）。用哪只手它自己挑，每一步都是一张能点开的卡片。 |
...
| 没有 API 的 App | 碰不到——VM 里有浏览器，但够不着你的手机 | Android 上有一个模型可以调用的无障碍 CLI（`android-a11y-cli`） | 把屏幕当成一双正式的手：视觉加无障碍树，先试 API 再上屏幕，登录由你接管，同一套审批——0.1.12 |
| 其他设备 | 多个客户端连同一台 VM；VM 不碰你的设备 | 只有装了它的那一台 | 一个智能体，跨你的所有设备：先是手机操作电脑（0.1.13），之后所有设备双向互通 |
